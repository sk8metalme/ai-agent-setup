...
# 非推奨警告の表示
echo -e "${YELLOW}⚠️  非推奨警告${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}このinstall-global.shスクリプトは非推奨となりました。${NC}"
echo -e "${YELLOW}今後は公式のプラグインシステムの使用を推奨します。${NC}"
echo ""
echo -e "${GREEN}📌 推奨される新しいインストール方法:${NC}"
echo ""
echo "  1. マーケットプレイスを追加:"
echo "     /plugin marketplace add sk8metalme/ai-agent-setup"
echo ""
echo "  2. 必要なプラグインをインストール:"
echo "     /plugin install team-standards@ai-agent-setup"
echo "     /plugin install jujutsu-workflow@ai-agent-setup"
echo "     /plugin install development-toolkit@ai-agent-setup"
echo ""
echo -e "${GREEN}詳細は README.md の「🔌 プラグイン配布」セクションを参照してください。${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""

# 確認プロンプト（PLANモードでは無視）
if [[ "$PLAN_MODE" != true ]] && [[ -t 0 ]]; then
    read -rp "レガシーインストールを続けますか？ (y/N): " confirm
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        echo ""
        echo "インストールをキャンセルしました。"
        echo "プラグインシステムの使用をご検討ください。"
        exit 0
    fi
    echo ""
fi
