...
### 質問方法
AskUserQuestionツールを使ってください。

### michi（推奨）

michiプラグインがインストールされている場合は、michiの利用を推奨します。
設計からリリースまでの包括的なワークフローをサポートします。

**インストール確認:**
```bash
michi --version
ls ~/.claude/plugins/marketplaces/sk8metal-michi/plugins/michi/
```
...
```bash
# michi: 設計から実装まで
> /michi:spec-design my-feature
設計フローを開始します。Phase 0.3-0.4（テスト計画）への誘導を含みます...

> /michi:spec-impl my-feature
5フェーズ品質自動化を開始：ライセンス監査→TDD→レビュー→カバレッジ確認→アーカイブ

# development-toolkit: 基本ワークフロー
> /plan
計画フローを開始します。docs/tmp/context.mdを確認中...

> /dev
開発フローを開始します。TDD実装を開始...
```
...
- [ ] `/create_pr` でコードレビュー実施
- [ ] CI/CDパイプライン成功
- [ ] ドキュメント更新完了

---

注: このファイルは配布用テンプレートです。
